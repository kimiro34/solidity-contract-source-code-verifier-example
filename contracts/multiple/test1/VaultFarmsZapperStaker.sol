...
contract VaultFarmsZapperStaker {
    ...
    function Deposit(address _for, uint256 _amount) private {
        Distribute();
        uint prevStakeFor = stake[_for];
        stake[_for] = stake[_for].add(_amount);
        if (S0[_for] == 0  || prevStakeFor == 0) {
            S0[_for] = S;
        } else {
            uint stakeRatio = stake[_for].mul(1000).div(prevStakeFor);
            S0[_for] = S.mul(stakeRatio).div(1000);
        }
        T = T.add(_amount);
    ...
    function MyReward(address _for) public view returns (uint256) {
        ...
        return _reward.div(10**9).sub(claimedReward[_for]);
    }
